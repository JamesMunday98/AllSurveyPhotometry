...
class getATLASforcedPHOT(object):
    ...
    def plot_and_save_data(a, string):
        ...
        plt.errorbar(MJDc,magc, yerr=dmagc, fmt=".g", label="c")
        plt.errorbar(MJDo,mago, yerr=dmago, fmt=".b", label="o")
        plt.xlabel("MJD");   plt.ylabel("Magnitude");   plt.gca().invert_yaxis()
        plt.savefig("ATLASmag.png")
        plt.clf()
        
        #np.savetxt("ATLAS_filtC_flux_and_err.dat", np.array([MJDc[~masked_c],magc[~masked_c],dmagc[~masked_c],uJyc[~masked_c], duJyc[~masked_c] ]).T)
        #np.savetxt("ATLAS_filtO_flux_and_err.dat", np.array([MJDo[~masked_o],mago[~masked_o],dmago[~masked_o],uJyo[~masked_o], duJyo[~masked_o] ]).T)
        
        np.savetxt("ATLAS_filtC_flux_and_err.dat", np.array([MJDc,magc,dmagc,uJyc, duJyc]).T)
        np.savetxt("ATLAS_filtO_flux_and_err.dat", np.array([MJDo,mago,dmago,uJyo, duJyo]).T)
